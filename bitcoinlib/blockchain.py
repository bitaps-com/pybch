...
class Transaction():
    ...
    def serialize(self, sighash_type = 0, input_index = -1, subscript = b''):
        ...
        if sighash_type &  SIGHASH_ANYONECANPAY : raw += b'\x01'
        else: raw += to_var_int(self.tx_in_count)
        for number, i in enumerate(self.tx_in):
            if (sighash_type &  SIGHASH_ANYONECANPAY) and (input_index != number): continue
            raw += i.outpoint[0]+i.outpoint[1].to_bytes(4,'little')
            if sighash_type == 0 or input_index == number:
                if sighash_type:
                    raw += to_var_int(len(subscript))
                    raw += subscript
                else :
                    raw += to_var_int(len(i.sig_script.raw))
                    raw += i.sig_script.raw
                raw += i.sequence.to_bytes(4,'little')
            else:
                raw += b'\x00'
                if (sighash_type&31) == SIGHASH_ALL: raw += i.sequence.to_bytes(4,'little')
                else: raw += b'\x00\x00\x00\x00'

        if (sighash_type&31) == SIGHASH_NONE: raw += b'\x00'
        else:
            if (sighash_type&31) == SIGHASH_SINGLE: raw += to_var_int(input_index + 1)
            else: raw += to_var_int(self.tx_out_count)
            for number, i in enumerate(self.tx_out):
                if (sighash_type&31) == SIGHASH_SINGLE and (input_index != number):
                    raw += b'\xff\xff\xff\xff\xff\xff\xff\xff\x00'
                else:

                    raw += i.value.to_bytes(8,'little')
                    raw += to_var_int(len(i.pk_script.raw))
                    raw += i.pk_script.raw

        raw += self.lock_time.to_bytes(4,'little')
        return raw
